          ...
          <tbody>
            {displayedRows.map((row) => {
              ...
              const investmentCells = {
                ...
                overrideSymbol: <td key="overrideSymbol" className="investment-column--override">{derived?.incomeItem ? <div className="readonly-cell readonly-cell--text">N.A.</div> : <AssetSelect value={row.newSymbol || row.symbol} options={symbolOptions} accountTaxStatus={rowTaxStatus} tickerMap={tickerMap} stateCode={stateCode} onChange={(value) => onChange(row.id, "newSymbol", value)} onJumpToAsset={onJumpToAsset} ariaLabel={`What-If asset for ${row.description || row.account || "investment row"}`} />}</td>,
