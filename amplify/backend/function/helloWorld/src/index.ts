...
type RequestBody =
  ...
  | ({ calc: "TAX_PLAN_2025" } & TaxPlan2025Input)
  | { calc: "TAX_CONFIG_2025" }
  | {
      calc: "LOCAL_TAX";
      taxableIncome: number;
      enabled?: boolean;
...
function resolveCorsOrigin(event: APIGatewayProxyEvent, restricted: boolean) {
  ...
  if (!authIsConfigured()) {
    ...
    return {
      response: jsonResponse(
        503,
        { error: "Portfolio access is disabled until Cognito or a sync token is configured." },
        origin
      ),
    };
  }
...
function workbookResponseForClient<T extends { workspaceId: string }>(result: T, requestedWorkspaceId: string, auth: AuthContext | null): T & { owner?: { authType: string } } {
  return {
    ...result,
...
function buildAssistantPortfolioContext(snapshot: unknown, userContent: string) {
  if (!snapshot || typeof snapshot !== "object") {
    return { querySymbols: extractTickerSymbolsFromText(userContent) };
  }
  ...
  return {
    ...
    holdings: Array.isArray(source.holdings)
      ? source.holdings.map((holding: any) => ({
          id: holding?.id,
    ...
    referenceTables: {
      tickers: Array.isArray(source.referenceTables?.tickers)
        ? source.referenceTables.tickers.map((ticker: any) => ({
      ...
      categories: Array.isArray(source.referenceTables?.categories)
        ? source.referenceTables.categories.map((row: any) => ({ name: row?.name }))
        : [],
      taxTreatment: Array.isArray(source.referenceTables?.taxTreatment)
        ? source.referenceTables.taxTreatment.map((row: any) => ({ label: row?.label }))
        : [],
      accountTaxType: Array.isArray(source.referenceTables?.accountTaxType)
        ? source.referenceTables.accountTaxType.map((row: any) => ({ taxStatus: row?.taxStatus }))
        : [],
      investmentType: Array.isArray(source.referenceTables?.investmentType)
        ? source.referenceTables.investmentType.map((row: any) => ({ name: row?.name }))
        : [],
    },
...
async function handlePublicReportRequest(
  event: APIGatewayProxyEvent,
  body: RequestBody
): Promise<APIGatewayProxyResult> {
  ...
  if (calc === "PUBLIC_REPORT_GET") {
    ...
    try {
      ...
      if (!report) return jsonResponse(404, { error: "Public report not found." }, origin);
      return jsonResponse(200, {
        report: {
          id: report.reportId,
          slug: report.slug,
  ...
  const payloadResult = readPublicReportPayload((body as { payload?: unknown }).payload);
...
export const handler = async (
  ...
  if (calc === "STATE_TAX_2025") {
    ...
    if (!stateTaxProfiles.some((profile) => profile.code === state)) {
      return jsonResponse(400, { error: `Unsupported state: ${state}` }, origin);
    }

    const result = calculateStateTax2025({
      state,
      filingStatus,
  ...
  if (calc === "FED_TAX_2025_COMBINED") {
    const usesGrossIncome = (body as any).ordinaryIncome !== undefined
      || (body as any).preferredIncome !== undefined
      || (body as any).deduction !== undefined;
    let taxableSplit: ReturnType<typeof splitFederalTaxableIncome2025>;

    if (usesGrossIncome) {
      const ordinaryIncome = readNonNegativeNumber((body as any).ordinaryIncome, "ordinaryIncome");
      if ("error" in ordinaryIncome) return jsonResponse(400, { error: ordinaryIncome.error }, origin);
      const preferredIncome = readNonNegativeNumber((body as any).preferredIncome, "preferredIncome");
      if ("error" in preferredIncome) return jsonResponse(400, { error: preferredIncome.error }, origin);
      const requestedDeductionMode = (body as any).deductionMode;
      ...
      if (requestedDeductionMode) {
        const aboveLineDeduction = readNonNegativeNumber((body as any).aboveLineDeduction ?? 0, "aboveLineDeduction");
        if ("error" in aboveLineDeduction) return jsonResponse(400, { error: aboveLineDeduction.error }, origin);
        const selectedDeduction = requestedDeductionMode === "standard"
          ? federalStandardDeduction2025(String((body as any).filingStatus || "mfj").toLowerCase() as FilingStatus)
          : readNonNegativeNumber((body as any).itemizedDeduction ?? 0, "itemizedDeduction");
        ...
        taxableSplit = splitFederalTaxableIncome2025(
          ordinaryIncome.value,
          preferredIncome.value,
          aboveLineDeduction.value + standardOrItemizedDeduction
        );
      } else {
        const deduction = readNonNegativeNumber((body as any).deduction, "deduction");
        if ("error" in deduction) return jsonResponse(400, { error: deduction.error }, origin);
        taxableSplit = splitFederalTaxableIncome2025(ordinaryIncome.value, preferredIncome.value, deduction.value);
      }
    } else {
      const ordinaryTaxable = readNonNegativeNumber((body as any).ordinaryTaxable, "ordinaryTaxable");
      if ("error" in ordinaryTaxable) return jsonResponse(400, { error: ordinaryTaxable.error }, origin);
      const prefTaxable = readNonNegativeNumber((body as any).prefTaxable, "prefTaxable");
      if ("error" in prefTaxable) return jsonResponse(400, { error: prefTaxable.error }, origin);
      taxableSplit = splitFederalTaxableIncome2025(ordinaryTaxable.value, prefTaxable.value, 0);
    }
  ...
  if (calc === "TAX_CONFIG_2025") {
    return jsonResponse(200, {
      calc,
      taxYear: 2025,
      states: stateTaxProfiles,
      localities: localTaxProfiles2025,
    }, origin);
  }

  if (calc === "TAX_PLAN_2025") {
    const filingStatus = String((body as any).filingStatus || "").toLowerCase();
    if (!isFilingStatus(filingStatus)) {
      return jsonResponse(400, { error: "filingStatus must be one of: single, mfj, mfs, hoh" }, origin);
    }
    const state = String((body as any).state || "").trim().toUpperCase();
    if (!state) return jsonResponse(400, { error: "TAX_PLAN_2025 requires state" }, origin);
    if (!stateTaxProfiles.some((profile) => profile.code === state)) {
      return jsonResponse(400, { error: `Unsupported state: ${state}` }, origin);
    }
    const localityId = String((body as any).local?.localityId || "none");
    if ((body as any).local?.enabled === true && !localTaxProfiles2025.some((profile) => profile.id === localityId)) {
      return jsonResponse(400, { error: `Unsupported locality: ${localityId}` }, origin);
    }
    const result = calculateTaxPlan2025({ ...(body as any), filingStatus, state });
