# Portfolio MCP Server

This MCP server gives ChatGPT access to the existing portfolio workbook data and tax calculation API. Read tools work alongside a small set of explicit write tools for adding or updating investment rows.

## What it exposes

- `get_workbook_overview`
- `list_investments`
- `get_investments_table`
- `get_tickers_table`
- `get_tax_treatment_table`
- `get_accounts_table`
- `get_reference_tables`
- `run_federal_tax_calculation`
- `run_california_tax_calculation`
- `search_portfolio_notes`
- `add_investment`
- `update_investment`
- `set_investment_checkbox`
- `select_investment_rows`
- `highlight_investment_rows`
- `update_reference_row`
- `upsert_reference_rows`
- `replace_reference_table`

The server reads from the existing live workbook API at:

- `https://j4evba8fpj.execute-api.us-west-2.amazonaws.com/portfolio/hello`

## Run locally

```powershell
cd C:\myapp\mcp-server
npm install
npm run build
npm run dev
```

By default it starts at:

- MCP endpoint: `http://localhost:8787/mcp`
- Health endpoint: `http://localhost:8787/health`

## Environment variables

- `PORT`: HTTP port, default `8787`
- `MCP_PATH`: MCP route, default `/mcp`
- `PORTFOLIO_API_BASE_URL`: workbook and tax backend URL
- `PORTFOLIO_WORKSPACE_ID`: default workspace, default `default`
- `PORTFOLIO_SYNC_TOKEN`: optional sync token used as `X-Portfolio-Sync-Token` so the MCP server can access the same user-scoped workbook as the spreadsheet export.

ChatGPT cannot reuse the Cognito login from your browser. To access your signed-in workbook, run the MCP server with a sync token that is mapped to the Cognito user who owns the workbook:

```powershell
cd C:\myapp\mcp-server
$env:PORTFOLIO_SYNC_TOKEN = Get-Content C:\myapp\tmp\portfolio-sync-token.txt -Raw
npm run dev
```

## Hosted Cloudflare Worker

The repo now includes a Cloudflare Worker entry point at `mcp-server/src/worker.ts`.
It serves `/mcp` from the same Worker that serves the React app static assets.

Required Worker secret:

- `PORTFOLIO_SYNC_TOKEN`: used as `X-Portfolio-Sync-Token` when the Worker calls the portfolio backend.

Recommended Worker secret:

- `MCP_AUTH_TOKEN`: protects the public MCP endpoint.
- `ALLOW_PUBLIC_MCP`: optional. Leave unset to require bearer auth by default. Set to `true` only if you intentionally want a public MCP endpoint.

For multi-user access, signed-in users should generate their own ChatGPT connector token from the React app. Those user tokens are stored hashed in the portfolio backend and are passed to the backend as `X-Portfolio-MCP-Token`; they are different from the Worker-level `MCP_AUTH_TOKEN`.

The public MCP URL will be one of these after deployment:

- `https://my-free-web-app.kmcclusk.workers.dev/mcp`
- `https://www.aftertaxus.com/mcp`
- `https://www.aftertaxus.com/mcp-v2`
- `https://www.aftertaxus.com/mcp-v3`
- `https://www.aftertaxus.com/mcp-v4`
- `https://www.aftertaxus.com/mcp-v5`

Use `/mcp-v5` for new ChatGPT connector setup if ChatGPT still shows an older cached tool list for `/mcp`, `/mcp-v2`, `/mcp-v3`, or `/mcp-v4`.

If `MCP_AUTH_TOKEN` is set and you use the shared admin auth, configure the MCP client with:

- URL: `https://www.aftertaxus.com/mcp`
- Bearer token: `YOUR_MCP_AUTH_TOKEN`

For normal users, use the URL and bearer token generated by the AfterTaxUS app instead. Do not use `PORTFOLIO_SYNC_TOKEN` as the bearer token value. Keep that token server-side only.

## Notes

- This server does not inherit the React app browser login. It authenticates to the portfolio backend with `PORTFOLIO_SYNC_TOKEN` when provided.
- Hosted MCP accepts bearer auth and user connector URLs with `mcp_token` in the query string.
- Use `set_row_highlight` when ChatGPT needs an explicit front-end row highlight action. `highlight_investment_rows` and `select_investment_rows` remain compatibility aliases.
- Compatibility: `update_investment` accepts `values.select`/`values.highlight`, and `set_investment_checkbox` accepts `field: "select"` or `"highlight"` to highlight rows through tools ChatGPT may already recognize.
- Write tools are intentionally limited to investment rows, checkbox updates, row highlighting, and reference-table edits. Do not expose this MCP server publicly without `MCP_AUTH_TOKEN` or equivalent access control.
- It talks to the same live backend used by the spreadsheet and React app.
- It does not need direct DynamoDB credentials as long as the workbook API remains available.
